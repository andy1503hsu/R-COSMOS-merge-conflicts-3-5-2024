%% This is an unfinished function! Currently, it is copy-pasted from
%% planetarySimulate3.m, just with a renamed function/file name

% Differences between Ark Chamber and Planetary tests:
% Existence of wall photons (*unchanging temperatures*)
% LED bar is stationary (no changing zenith/azimuth angle)
    % This change should be taken care of in getPhotonValues, not here

% ---Reflectivity, not exiting---
% Side of domain are NOT transmitting. Instead, they are diffusively reflective
% No exiting photons -- Ceiling is reflective
    % Reflectivity is dependent on whether photon strikes SLI (specular) or not SLI (diffusive)
% Bottom of domain is modeled as diffusivley reflective

function [aData, eData, energyData] = arkSimulate(sData, photons) %#codegen
        ...
        while 1 % Keep looping until specific conditions are met
            ...
            if onePhoton(4) == 1 % Ray-tracing region
                if onePhoton(7) > 0 && (photon_slope > maxSlope || photon_slope < minSlope) % Guaranteed to not hit a surface
                    ...
                    [xSc,zSc] = intersectSurfaceNew(x1,z1,x2,z2,xi,zi,xf,zf,zM,uC,x34,z34,determinant34);
